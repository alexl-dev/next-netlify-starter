...
/** Search for a town by name, so a gauge can be found without standing in it. */
export async function findPlaces(query) {
  const trimmed = String(query || '').trim();
  if (trimmed.length < 2) return [];
  const res = await fetch(`/api/places?q=${encodeURIComponent(trimmed)}`);
  if (!res.ok) throw new Error('Could not reach the place directory');
  const json = await res.json();
  return json.places || [];
}

/**
 * Stations already bound to other waters.
 *
 * Most people fish a handful of rivers and rebind the same few gauges — a
 * second stretch of the same river usually wants the gauge the first stretch
 * already uses. Offering those first turns the common case from "find my
 * position, search, read a list" into one tap.
 *
 * `excludeWaterId` drops the water being edited, so its own current station is
 * not offered back to it as a suggestion.
 */
export function stationsInUse({ waters = [], kind = null, excludeWaterId = null } = {}) {
  const seen = new Map();

  for (const water of waters) {
    if (!water.station || !water.station.id) continue;
    if (water.id === excludeWaterId) continue;
    // A tide station and a river gauge are not interchangeable.
    if (kind && water.station.kind && water.station.kind !== kind) continue;

    const key = `${water.station.provider}:${water.station.id}`;
    const existing = seen.get(key);
    if (existing) {
      existing.usedBy.push(water.name);
      continue;
    }
    seen.set(key, {
      ...water.station,
