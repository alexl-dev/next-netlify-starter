...
// `source` records how the coordinates were obtained — 'gps' when the device
// measured them, 'map' when a person placed them, 'recent'/'water' when they
// were reused from something already in the log. Old pins carry null, which
// means nobody knows. It is here so a hand-placed spot can never be mistaken
// for a fix later, when someone asks these records a harder question.
export const newPin = ({ tripId, lat, lon, name = '', accuracyM = null, source = null }) => ({
  id: uid(),
  tripId,
  lat,
  lon,
